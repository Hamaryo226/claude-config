...
分量は目安。必要な制約を失うまで削ったり、目安まで内容を足したりしない。

| ファイル | 目安 | 長くなったときの対処 |
| --- | --- | --- |
| `CLAUDE.md` | 120 行 | rules に逃がす前に、まず削る |
| `.claude/rules/*.md` | 各 60 行 | ルールを絞る。ファイルを増やさない |
| `.claude/skills/*/SKILL.md` | 各 100 行が目安 | 条件付きの詳細を `references/` に分け、読む条件付きの Markdown リンクで参照する |
...
`paths` が書けないものは rules にしない。常時指示か、作業時に参照する資料か、記載不要かを選ぶ。
...
起動設定は対象ランタイムの仕様に従い、既存設定を保持する。スキルの発見・読み込みは操作の承認ではない。
手動専用の指定と、実操作に必要な承認を混同しない。
